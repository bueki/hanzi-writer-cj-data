...
const orientPaths = (paths: any, approximation_error?: any) => {
...
const splitPath = (path: any) => {
  ...
  for (let i = 0; i < terms.length; i++) {
    ...
    assert('LMQZC'.indexOf(command) >= 0, command);
    if (command === 'M' || command === 'Z') {
      ...
      start = [parseFloat(terms[i + 1]), parseFloat(terms[i + 2])];
    ...
    if (command === 'Q') {
      assert(i < terms.length - 2, `Missing point on path: ${path}`);
      control = [parseFloat(terms[i + 1]), parseFloat(terms[i + 2])];
      assert(Point.valid(control));
      i += 2;
    }
    if (command === 'C') {
      assert(i < terms.length - 3, `Missing point on path: ${path}`);
      control = [parseFloat(terms[i + 1]), parseFloat(terms[i + 2])];
      assert(Point.valid(control));
      i += 4;
    }
    assert(i < terms.length - 2, `Missing point on path: ${path}`);
    const end = [parseFloat(terms[i + 1]), parseFloat(terms[i + 2])];
...
svg.convertCommandsToPath = (commands: any) => {
  const terms: any[] = [];
...
svg.getPolygonApproximation = (path: any, approximation_error?: any) => {
  const result: any[] = [];
...
svg.polygonContainsPoint = (polygon: any, point: any) => {
  const x = point[0];
  const y = point[1];
  let crossings = 0;
  for (let i = 0; i < polygon.length; i++) {
